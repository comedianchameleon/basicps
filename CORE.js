...
exports.CORE = {
	nameColor: function (name, bold, userGroup) {
		...
		return (userGroup ? userGroupSymbol : "") + (bold ? "<b>" : "") + "<font color=" + CORE.hashColor(name) + ">" + (Users(name) && Users(name).connected && Users.getExact(name) ? Chat.escapeHTML(Users.getExact(name).name) : Chat.escapeHTML(name)) + "</font>" + (bold ? "</b>" : "");
	},
	// usage: CORE.nameColor(user.name, true) for bold OR CORE.nameColor(user.name, false) for non-bolded.
	...
	// format: CORE.messageSeniorStaff('message', 'person')
	//
	// usage: CORE.messageSeniorStaff('Mystifi is a confirmed user and they were banned from a public room. Assess the situation immediately.', '~Server')
	//
...
// last two functions needed to make sure CORE.regdate() fully works
